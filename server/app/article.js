...
router.get('/lst', modern(async function (req, res) {
    try {
        ...
        if(req.query.cateid){
            if(parseInt(req.query.cateid)>0 || parseInt(req.query.cateid)===0){
                 cateid = req.query.cateid
            }else{
                keywords = req.query.cateid
            }
        }
        let restult = {
            code: '200',
        ...
        let rows = await query(`select a.id,title,author,pic,keywords,cateid,catename,FROM_UNIXTIME(time,'%Y-%m-%d') as create_time  from tp_article a join tp_cate c on a.cateid=c.id ${where} order by id desc limit ${pageinfo},${pageSize}`);//分页查询
        for(let i in rows){
            rows[i].keywords = rows[i].keywords.split(',')
        }
...
router.get('/find', modern(async function (req, res) {
    ...
    try {
        ...
        let sql = "select a.id,title,author,state,pic,cateid,catename,content,keywords,`desc`,FROM_UNIXTIME(time,'%Y-%m-%d') as create_time,click from tp_article a join tp_cate c on a.cateid=c.id";
        let rows = await query(`${sql} where a.id=${id}`);
        let art = rows[0];
        art.keywords=art.keywords.split(',');
        res.json({
            code: '200',
            msg: '请求成功',
            data: art
...
router.get('/hots', modern(async function (req, res) {

    try {

        let rows = await query(`select id,title from tp_article order by click desc limit 10 `);
...
router.get('/recoms', modern(async function (req, res) {

    try {

        let rows = await query(`select id,title from tp_article where state=1 order by click desc limit 10 `);
...
router.get('/upOfClick', modern(async function (req, res) {
    let id = parseInt(req.query.id) || 0;
    ...
    try {
        ...
        let rows = await query(`update tp_article set click=click+1 where id=?`,id);
